...
**Deux formats, deux tirages, un seul moteur.** `MODES` dans `js/sim.js` est le produit de deux réglages, et `modeDe(format, tirage)` donne la clé. Le **format** dit combien de cases : **Complet** (les 23, 95,5 M$) ou **Express** (six cases — un trio, une paire, un partant — le reste fourni par une vraie équipe tirée au hasard, hors plafond et non modifiable, sous 34 M$, la médiane mesurée de ce qu'une vraie équipe met sur ces six cases-là). Le **tirage** dit d'où viennent les joueurs, et dans les deux cas c'est l'**unité équivalente** : on aligne la vraie équipe sortie par `autoRoster` et on ne montre que ce qu'elle met à la place qu'on comble. **Vestiaire** : une équipe, son unité entière (son premier trio pour ton premier trio, ses deux gardiens, ses trois réservistes), tu en signes un, **aucune relance**. **Loto** : trois équipes, le joueur de la case exacte de chacune, tu choisis, et les relances (huit en complet, trois en express) relancent les trois. JP : *tu dois construire avec seulement les joueurs des trios équivalents des équipes roulées* ; *un nouveau mode que le jeu te propose l'ailier numéro un de trois équipes, ça ferait plus loto, et les rerolls seraient pertinents*. Le mode « Par unité » et les relances d'année, d'équipe et de passe ont disparu avec le vestiaire de trente cartes. `uniteEquivalente` et `joueurEquivalent` (`js/sim.js`) sont les deux primitives ; `candidats()` (`js/game.js`) dérive la main à chaque rendu du tirage et de la **case courante** — la case visée, sinon la première vide — donc viser une autre case recompose la main du même tirage sans relancer. La roulette refuse une main où rien ne tient dans le budget du choix tant qu'elle peut faire autrement (vingt essais, puis sous le plafond restant, puis plaçable) : sans relance, une main de trois contrats trop chers serait une impasse, pas un tirage. Le moteur, les 23 cases, le malus de zone et la ligue de 32 équipes ne changent jamais.
...
**La main est une rangée de deux ou trois cartes, pas un vestiaire.** Le tri, la recherche, les sept pastilles de position, le bouton « Par poste / Tous » et les six colonnes qu'on balayait sont partis avec le vestiaire de trente cartes : on ne cherche pas dans trois cartes, on les lit, dans l'ordre de l'unité (AG, C, AD ; DG, DD ; partant, auxiliaire). Le volet s'appelle « La main », son titre reste visible partout, et le conteneur `pool` reste nommé parce que la carte s'y mesure.
...
**Toujours AG, C, AD, DG, DD, G.** Le même ordre et les mêmes sigles partout : la main, les cases du tableau de profondeur, les bandeaux de carte, les tables d'équipe. Le mot complet vit dans l'infobulle. La bande de secours d'impasse a son propre conteneur, `#poolNotice`, au-dessus de la main.
...
2. Auto-draft conscient du budget : à chaque tour il lit le plafond restant, calcule ce qu'il peut mettre sur ce choix sans passer sous le plancher pour les cases suivantes, et clique le premier `.pcard .btn-sign:not([disabled])` de la main qui tient dans ce budget. Sinon le moins cher ; en dernier recours il clique `#freeCapBtn`, le bouton de la bande de secours qui retire le plus gros contrat
