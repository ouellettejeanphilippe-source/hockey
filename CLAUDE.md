...
**Les punitions et les unités spéciales se jouent comme un petit match dans le match** (`jouerSoixanteMinutes`, MOTEUR.md 4.7). Chaque équipe prend des punitions au rythme de son époque (`AVANTAGES_EPOQUE`, repère public : ≈ 4 par match en 1970, 5,3 au milieu des années 1980, 5,8 en 2005-06, 3 aujourd'hui) multiplié par son **indiscipline** — les minutes de punition par match de ses patineurs habillés, relatives au régulier moyen de leur saison (colonne [7] de `SEASON_LANCERS`) — et le plus puni va au banc. Chaque mineure ouvre une fenêtre de deux minutes : l'avantage tire `AN_TIRS_MIN` par minute avec ses deux unités (trois attaquants et deux défenseurs rangés par création et finition, 65/35), le désavantage tire peu avec ses deux unités de quatre meilleures cotes défensives (60/40), et le premier but ferme la fenêtre. Le cinq contre cinq se joue sur les minutes qui restent. **Ce qui a été trouvé en mesurant** : un joueur d'avantage tirait deux fois, parce que son volume réel contient déjà ses tirs d'avantage — Bondra 2001-02 faisait 93 buts au lieu de 46 et l'erreur systématique de `check_feuilles.mjs` passait de 20 à 36 %. `PART_AN_TIRS` (35 % pour la première unité, 15 % pour la deuxième) retire cette part à forces égales, la pointe tire plus en avantage (`PART_LANCERS_D_AN`), le tireur d'avantage se tire sur la racine de son volume, et la création n'y compte qu'à moitié. **JP a demandé un avantage qui compte** : `AN_QUALITE` = 1,25 et `FE_QUALITE` = 1,03, donc **28,6 % des buts se marquent en avantage numérique** (0,89 par équipe par match), au-dessus des 20 à 22 % réels, et c'est voulu. Mesuré sur cinq ligues : 28,2 lancers, 3,10 buts, 4,6 punitions par équipe par match, erreur systématique par joueur 20,3 % (20,1 avant), Selanne 2006-07 à 44-47 buts pour 48 réels. Le +/- ne compte pas les buts en avantage (règle de la ligue). **Ce qui n'est pas encore mesuré sur le dépôt, mais le build sait maintenant le prendre** (`RATINGS_VERSION` 24) : `skater/summary` donne les buts et points en avantage et en désavantage (`ppg`, `ppp`, `shg`, `shp` sur chaque patineur — absents tant que l'Action n'a pas rebâti les shards), et `team/powerplay` donne par équipe les occasions et les buts en avantage depuis 1977-78 (bloc `an` du shard, que `rerateShard` conserve). Quand ces champs existent, le moteur **les lit au lieu de deviner** : les unités d'avantage viennent des points en avantage, celles de désavantage des points en désavantage, la part d'avantage d'un joueur est ses buts en avantage sur ses buts, et les occasions par saison remplacent les repères d'époque (colonne [9] de `SEASON_LANCERS`, posée par `build_lancers.mjs`). Sans eux, tout retombe sur ce qui précède. **La marche à suivre** : onglet Actions → « Bâtir les données » → mode `full` (l'API n'est pas joignable d'ici), puis `node scripts/build_lancers.mjs`, puis `LIGUES=5 node scripts/check_feuilles.mjs`, qui compare alors la part des buts d'avantage simulée à la réelle, joueur par joueur — et c'est ce chiffre qui règle `AN_QUALITE` pour de bon.
