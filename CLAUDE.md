...
**La couleur d'équipe porte du texte à un seul endroit : le bandeau de carte, avec une encre mesurée.** La règle disait autrefois « jamais », faute de savoir lire la couleur ; `getTeamInk` (`js/logos.js`) la mesure maintenant — elle calcule les deux rapports de contraste WCAG et garde le meilleur. Sur les 32 équipes, parce que `getTeamAccent` éclaircit tout jusqu'à une luminance de 0,30, le blanc ne dépasse jamais **2,8:1**, sous le seuil lisible de 4,5:1, pendant que le bleu nuit donne de **7,5:1** (Vancouver, Philadelphie) à **12,1:1** (Boston, Nashville). L'encre foncée gagne donc partout, et le calcul reste pour que ça tienne si l'éclaircissement change. **Partout ailleurs la règle ne bouge pas** : `--team-primary` et `--team-accent` ne teintent que fonds, bordures et lueurs, et les états sélectionnés (chips, segments d'options) gardent `--ui-accent`, fixe.
